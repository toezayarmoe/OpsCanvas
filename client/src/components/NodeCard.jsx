...
export default function NodeCard({ data, type, selected }) {
  ...
  return (
    <div className={`w-64 rounded-2xl border bg-[#10151f]/95 shadow-panel transition ${selected ? "border-emerald-400 shadow-glow" : "border-zinc-800"}`}>
      ...
      <div className="flex items-start gap-3 border-b border-zinc-800/80 p-3">
        <div className="mt-0.5 rounded-lg bg-zinc-800 p-2 text-zinc-200"><Icon size={15} /></div>
        ...
        <span className={`mt-1 h-2.5 w-2.5 rounded-full ${statusClass[data.status || "idle"]}`} />
      </div>
      <div className="p-3">
        <div className="truncate rounded-lg border border-zinc-800 bg-[#090c13] px-2.5 py-2 font-mono text-[11px] text-zinc-400">
          {summary || "Not configured"}
        </div>
      </div>
      <Handle type="source" position={Position.Bottom} className="!h-2.5 !w-2.5 !border-2 !border-[#10151f] !bg-emerald-400" />
    </div>
  );
}
